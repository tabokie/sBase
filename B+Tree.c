...
//#define Log()                       ({printf("%s ",__func__);printf("\n");})//PutInts(GLOBAL_Pointer->root->key,GLOBAL_Pointer->root->size);})
...
index_page firstIndexPage(void){
    ...
    first->size++;
    first->key[0]=NINF;
    first->pointer[0]->size++;
    first->pointer[0]->key[0]=NINF;
    return first;
}
...
int splitLeaf(leaf_page l){//related to father
    ...
    printf("index page to split(l->father): %d\n",l->father);
    if(l->father->size>=M+1&&!splitIndex(l->father))Error(split index error!);
    printf("addr for now index pages: %d, %d\n",(int)left->father,(int)right->father);
    //Error(0);
    For(i,0,left->father->size)printf("%d ",left->father->key[i]);
    printf("\n");
...
int splitIndex(index_page l){// this routine destory previous index page
    ...
    int i;
    printf("left(%d):",(int)left);
    For(i,0,left->size)printf("%d ",left->key[i]);
    printf("\n");
    printf("right(%d):",(int)right);
    For(i,0,right->size)printf("%d ",right->key[i]);
    printf("\n");
    ...
    //int i;
    printf("addrs:");
    For(i,0,(M+1))printf("%d ",l->pointer[i]);
    printf("\n");
    printf("fathers:");
    For(i,0,(M+1))printf("%d ",(l->pointer[i])->father);
    printf("\n");
    For(i,0,size1)(l->pointer[i])->father=left;
    For(i,(M+1)/2,(M+1)/2+size2)(l->pointer[i])->father=right;
    printf("fathers:");
    For(i,0,(M+1))printf("%d->%d ",l->pointer[i],(l->pointer[i])->father);
    printf("\n");
    //change index
